...
이 문제는 제목이 이상해서 도대체 뭘까 했는데 문제 설명만 요란했다!
'다익스트라' 알고리즘을 사용하여 풀었습니다.
잃는 금액을 최소로 한다는 조건 때문에 node클래스 내에서 compareTo 메서드를 재정의 해주었습니다.

> **간단한 설명**

잃는 금액을 최소로 한다는 조건 때문에 node클래스 내에서 compareTo 메서드를 재정의 해주었습니다.
그리고는 다익스트라 알고리즘을 우선순위 큐를 이용하여 구현하였고, 코드 자체는 직관적이라 이해하긴 쉬울 거에요.
간단한 주석도 달아두었습니다 ~