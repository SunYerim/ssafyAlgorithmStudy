...
사실 해당문제는 반년전쯤 한 번 푼 적이 있는 문제였어서 어느정도 기억은 있었다..
근데 만약 이런 문제를 처음 접한다면 ..? 어떤 알고리즘을 써야할까 좀 막막한 생각이 들었을 수도 있었겠다 라는 생각을 하면서 문제를 다시 풀어보았다.
지용이가 플로이드-워셜을 안 쓰고 풀었다고 했는데 조금 궁금합니다 ..!

> **간단한 설명**

우선 본인의 키 순서를 알 수 있으려면 -> n명의 학생이 키 순서를 비교할때, count가 n-1이라는 것은 본인을 제외한 모든 학생과 비교가 됐다는 것
그래서, count가 n-1이되면 answer을 1 증가시켰다.