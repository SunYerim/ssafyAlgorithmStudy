...
오늘 수업시간때 트리를 배우고나서 바로 문제 목록해서 하나 찾아서 풀었던 문제 !!
완전 이진 트리를 재귀적으로 탐색하여 푸는 방향으로 접근해서 문제를 해결했다.

--- 
> **간단한 설명**

배열의 '가운데 요소'를 기준으로 왼쪽 부분 배열과 오른쪽 부분 배열로 나눠 재귀적으로 탐색.
원소가 루트가 되고, 루트를 기점으로 왼쪽, 오른쪽 배열이 또 형성된다.

- 중위 순회를 하면서 레벨 별로 노드를 저장한다.
- level 2차원 리스트를 사용하여 각 레벨별 노드들을 저장하고, 이를 순서대로 출력하는 방식으로 코드를 작성했다.