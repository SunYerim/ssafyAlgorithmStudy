...
- 노드의 왼쪽 -> 본인 -> 오른쪽으로 순회 ===> 트리 중위탐색이구나!
- 중위탐색 구현하는 방법이,, 감이 안 잡힘,,,, 수업 좀 들을걸.....
- 트리 중위 탐색하는 방법 인터넷으로 검색함

    재귀 돌리는데 현재 노드에서 왼쪽 자식 먼저 재귀 돌리고, 그 다음 현재 노드 순회한 정보 기록, 그 다음 오른쪽 자식 재귀 돌리면 중위 순회!

- 중위 순회하는 순서대로 입력받은 노드 번호들 쏙쏙!
- 트리의 깊이에 따라 출력