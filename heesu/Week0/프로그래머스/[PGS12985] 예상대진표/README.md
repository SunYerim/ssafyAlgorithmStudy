...
## 접근 방법
테스트케이스 데이터 대로 그냥 그림 그려봄
N이 2의 지수 승으로 주어진다 -> 경기 카운트는 while n 하고, n/=2로.
다음 경기에 주어지는 번호는 ceil(현재 번호 / 2)
경기를 붙는 경우는 주어지는 번호가 같은 경우 -> 그 때의 경기 카운트 값 리턴함

## 더 알고 싶은 점
경기 카운트 하는데 answer 변수를 따로 쓰지 않고, n을 이용해서 동시에 카운트 하는 방법?
귀찮아서 더 생각 안 해봄
